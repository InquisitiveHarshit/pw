    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#313131]/50 mt-1">{blogs.length} post{blogs.length !== 1 ? "s" : ""} published</p>
        </div>
        <button
      ...
      {/* Feedback */}
      {success && <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 text-sm font-semibold text-emerald-700">✅ {success}</div>}
      {error && <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm font-semibold text-red-600">⚠️ {error}</div>}

      {/* Blog Form */}
      {showForm && (
        <div className="bg-white rounded-2xl border border-[#C7C0AE]/30 shadow-sm p-6">
          <h3 className="font-extrabold text-[#313131] font-vietnam mb-5">
            {editId ? "Edit Blog Post" : "Write New Blog Post"}
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-[#313131]/50 uppercase tracking-wider mb-1.5">Title *</label>
              <input
                required
                value={form.title}
            ...
            <div>
              <label className="block text-xs font-bold text-[#313131]/50 uppercase tracking-wider mb-1.5">Excerpt (short summary)</label>
              <input
                value={form.excerpt}
            ...
            <div>
              <label className="block text-xs font-bold text-[#313131]/50 uppercase tracking-wider mb-1.5">Content *</label>
              <textarea
                required
                rows={10}
                value={form.content}
                onChange={(e) => setForm({ ...form, content: e.target.value })}
                className="w-full border border-[#C7C0AE]/50 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#FFA100] transition-colors resize-none"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-[#313131]/50 uppercase tracking-wider mb-1.5">Tags (comma separated)</label>
                <input
                  value={form.tags}
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-3 cursor-pointer">
                  <div
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${form.isPublished ? "bg-[#FFA100]" : "bg-gray-300"}`}
                  >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-[#C7C0AE]/30 p-12 text-center">
          ...
          <p className="text-sm text-[#313131]/50 mt-1">Click "+ New Blog Post" to publish your first article.</p>
        </div>
      ) : (
              ...
              {blogs.map((blog) => (
                <tr key={blog._id} className="hover:bg-[#FAF1E6]/30 transition-colors">
                  <td className="px-5 py-4">
                    ...
                    {blog.excerpt && <p className="text-[11px] text-[#313131]/40 mt-0.5 truncate max-w-xs">{blog.excerpt}</p>}
                  </td>
                  <td className="px-5 py-4 text-[#313131]/60">{blog.author?.name || "—"}</td>
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-1">
                      {(blog.tags || []).slice(0, 3).map((tag) => (
                        <span key={tag} className="px-2 py-0.5 bg-[#FAF1E6] text-[#313131]/60 text-[10px] font-semibold rounded-full">{tag}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                      blog.isPublished ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"
                    }`}>
